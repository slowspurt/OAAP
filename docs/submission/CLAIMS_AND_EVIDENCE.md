...
| C05 | The receiver compares retries exactly and preserves literal text in its intended storage path. | Available: receiver source and local apostrophe/formula regression tests; [native test procedure](../../receiver/VERIFY.md). Operator reports a native Google failure: a tested leading apostrophe was removed and an identical retry failed. Fix/retest evidence pending; see [finding](NATIVE_FINDING.md). | Source and mocks do not establish real `getValues()` / `getFormulas()` behavior. |
